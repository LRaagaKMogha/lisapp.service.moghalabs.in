...
{
    public class StrainInventoryValidator : BaseValidator<StrainInventory>
    {
        public StrainInventoryValidator(HttpContext httpContext) : base(httpContext)
        {
            RuleFor(x => x)
                .Must(x =>
                {
            ...
            RuleFor(x => x.StrainLotNumber).NotEmpty();
        }
    }
}
