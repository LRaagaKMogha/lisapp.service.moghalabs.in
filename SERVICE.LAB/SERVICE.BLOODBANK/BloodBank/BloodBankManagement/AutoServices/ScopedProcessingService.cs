    ...
    internal class ScopedProcessingService : IScopedProcessingService
    {
        ...
        public async Task DoWork(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                var isChecked = await startupService.CheckSRDData();
                var response = await startupService.CheckForExpiration();
                var errorMessage = response.IsError ? "-" : "Errors";
                _logger.LogInformation("Executed Expiration Checks", errorMessage);
                var updateinventories = await startupService.UpdateInventoryStatus();
                errorMessage = updateinventories.IsError ? "-" : "Errors";
                _logger.LogInformation("Executed Inventory Status Checks", errorMessage);
