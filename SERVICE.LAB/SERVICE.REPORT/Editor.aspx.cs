...
namespace DEV.ReportService.Editor
{
    public partial class Editor : System.Web.UI.Page
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    if (Request.QueryString["data"] != null)
                    {
                        ...
                        string ordertemplateno = string.Empty;
                        string tempType = hdnTempType.Value != null && hdnTempType.Value != "" ? hdnTempType.Value : data[5];
                        if (hdnTempType.Value == "M")
                        {
                            ordertemplateno = hdntemplateno.Value != null && Convert.ToInt32(hdntemplateno.Value) > 0 ? hdntemplateno.Value : data[3];
                        }
                        else if (hdnTempType.Value == "O")
                        {
                            ordertemplateno = hdnOrderlistNo.Value != null && Convert.ToInt32(hdnOrderlistNo.Value) > 0 ? hdnOrderlistNo.Value : data[3];
                        }
                        else
                        {
                            ordertemplateno = data[3];
                        }
                        VenueNo = data[0];
                        VenueBranchNo = data[1];
                        PatientVisitNo = data[2];
                        OrderListNo = ordertemplateno;
                        ServiceNo = data[4];
                        Type = tempType;
                        if (Type == "O")
                        {
                            ...
                            if (File.Exists(file))
                            {
                                ...
                                ASPxRichEdit1.DocumentId = Guid.NewGuid().ToString();
                                hdnDocumentId.Value = ASPxRichEdit1.DocumentId;
                            }
                        }
                        else if (Type == "T")
                        {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            if (Request.QueryString["data"] != null)
            {
                ...
                if (Type != null && Type.ToUpper() == "M")
                {
                    ...
                    if (hdnDocumentId.Value != null && hdnDocumentId.Value != null && hdnDocumentId.Value != "")
                    {
                        DocumentManager.CloseDocument(hdnDocumentId.Value);
                    }
                }
                else
                {
        ...
        public void LogInfo(string data)
        {
            ...
            if (!File.Exists(folderpath + "\\" + "TemplateService.txt"))
            {
                File.Create(folderpath + "\\" + "TemplateService.txt").Dispose();
            }
            using (StreamWriter writer = File.AppendText(folderpath + "\\" + "TemplateService.txt"))
            {
                writer.WriteLine("Log On: " + datetime + " Actual Data: " + data);
            }
        }
